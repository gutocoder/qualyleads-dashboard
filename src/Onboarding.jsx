...
export default function Onboarding() {
  ...
  const webhookUrl = clientId ? `${BACKEND}/zapier/lead?client=${clientId}` : `${BACKEND}/zapier/lead`;

  const bookingPlaceholder =
    bookingTool === "calendly" ? "https://calendly.com/yourname/demo" :
    bookingTool === "google" ? "https://calendar.google.com/calendar/appointments/..." :
    "https://your-booking-link.com";

  function step1Next() {
    if (!businessName || !industry || !ownerName || !email) { setError("Please fill in all fields."); return; }
    setError(""); setStep(2);
  }
  ...
  async function sendTestLead() {
    ...
    try {
      const { data: clientData, error: clientErr } = await supabase
        .from("clients")
        .insert({ business_name: businessName, industry, language, owner_name: ownerName, email, phone, booking_url: bookingUrl || null, status: "active" })
        .select("id").single();
        ...
        <div style={{ background:BG, border:`1px solid ${BORDER}`, borderRadius:16, padding:32 }}>
          ...
          {step===1 && <>
            ...
            <div style={{ marginBottom:16 }}>
              ...
              <div style={{ fontSize:12, color:MUTED, marginTop:4 }}>
                {language==="nl" ? "🇳🇱 Qualy will text leads in Dutch via your NL number" : "🇺🇸 Qualy will text leads in English via your US number"}
              </div>
            </div>
            <div style={{ marginBottom:16 }}>
          ...
          {step===2 && <>
            ...
            <div style={{ marginBottom:16 }}>
              ...
              <div style={{ display:"flex", gap:8, marginTop:6 }}>
                {BOOKING_TOOLS.map(t=>(
                  <button key={t.value} onClick={()=>setBookingTool(t.value)} style={{ flex:1, height:38, borderRadius:8, border:`1.5px solid ${bookingTool===t.value?ACCENT:BORDER}`, background:bookingTool===t.value?"#f0fdf4":BG, color:bookingTool===t.value?ACCENT:TEXT, fontSize:13, fontWeight:bookingTool===t.value?600:400, cursor:"pointer", fontFamily:"inherit" }}>
                    {t.label}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom:24 }}>
              <label style={lbl}>Booking link <span style={{ fontWeight:400, color:MUTED }}>(optional)</span></label>
              <input value={bookingUrl} onChange={e=>setBookingUrl(e.target.value)} placeholder={bookingPlaceholder} style={inp} />
            </div>
            {error && <div style={{ fontSize:13, color:"#dc2626", background:"#fef2f2", border:"1px solid #fecaca", borderRadius:8, padding:"10px 14px", marginBottom:16 }}>{error}</div>}
          ...
          {step===3 && <>
            ...
            <div style={{ fontSize:13, color:MUTED, lineHeight:1.8, marginBottom:24 }}>
              ...
              • <strong>Google Forms:</strong> Use Apps Script → onFormSubmit
            </div>
            <div style={{ display:"flex", gap:10 }}>
          ...
          {step===4 && <>
            ...
            <div style={{ background:language==="nl"?"#eff6ff":"#f0fdf4", border:`1px solid ${language==="nl"?"#bfdbfe":"#bbf7d0"}`, borderRadius:8, padding:"10px 14px", marginBottom:16, fontSize:13 }}>
              {language==="nl" ? "🇳🇱 Test SMS will be sent in Dutch via +31 97010274977" : "🇺🇸 Test SMS will be sent in English via your US number"}
            </div>
            <div style={{ marginBottom:24 }}>
              <label style={lbl}>Test phone number *</label>
              <input value={testPhone} onChange={e=>setTestPhone(e.target.value)} placeholder={language==="nl"?"+31612345678":"+16125551234"} style={inp} />
